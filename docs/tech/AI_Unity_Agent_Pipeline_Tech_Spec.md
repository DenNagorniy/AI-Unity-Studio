...
### Agent Scorekeeper
`tools/gen_agent_scores.py` использует те же логи и вычисляет для каждого агента
успешность, частоту автофиксов и эскалаций, долю участия в успешных фичах и
тренд по последним пяти запускам. Результат сохраняется в `agent_scores.json` и
страницу `agent_scores.html`.
