...
Скрипт автоматически устанавливает Python и зависимости, запускает `env_setup.py`,
загружает Blender CLI и InvokeAI, скачивает модель Stable Diffusion, при пустом
`OPENAI_API_KEY` ставит Ollama и нужную LLM‑модель, а затем запускает
`ai_unity_studio_launcher.py`.
