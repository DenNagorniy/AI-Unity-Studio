...
Всё содержимое будет помещено в каталог `external/`. После завершения откроется
`ai_unity_studio_launcher.py` на [http://localhost:8000/overview](http://localhost:8000/overview).
